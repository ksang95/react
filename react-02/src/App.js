...
class App extends Component {
    ...
    getData = async () => {
        ...
        try {
            //API 호출하고. axios 라이브러리 필요.
            //axios.defaults.withCredentials=true;

            const response = await axios.get(`/v2/candles/trade:5m:t${pair}/hist?limit=288`);
            ...
            const data = response.data.map(
                ...
                (candle) => ({
                    date: moment(candle[0]).format('LT'), //시간만 나타나도록 설정. moment 라이브러리 필요.
                    value: candle[2]
                })
            ).reverse(); //역순으로 받아오게 되므로 순서를 반대로.
